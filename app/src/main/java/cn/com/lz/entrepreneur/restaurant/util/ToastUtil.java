...
public final class ToastUtil {
    ...
    private static int yOffset         = (int) (64 * App.getInstance().getApplicationContext().getResources().getDisplayMetrics().density + 0.5);
    ...
    public static void setView(@LayoutRes final int layoutId) {
        LayoutInflater inflate = (LayoutInflater) App.getInstance().getApplicationContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    ...
    private static void show(@StringRes final int resId, final int duration) {
        show(App.getInstance().getApplicationContext().getResources().getText(resId).toString(), duration);
    }
    ...
    private static void show(@StringRes final int resId, final int duration, final Object... args) {
        show(String.format(App.getInstance().getApplicationContext().getResources().getString(resId), args), duration);
    }
    ...
    private static void show(final CharSequence text, final int duration) {
        ...
        if (sViewWeakReference != null) {
            final View view = sViewWeakReference.get();
            if (view != null) {
                sToast = new Toast(App.getInstance().getApplicationContext());
                sToast.setView(view);
        ...
        if (!isCustom) {
            if (messageColor != DEFAULT_COLOR) {
                ...
                sToast = Toast.makeText(App.getInstance().getApplicationContext(), spannableString, duration);
            } else {
                sToast = Toast.makeText(App.getInstance().getApplicationContext(), text, duration);
            }
        }
