...
//        Log.i("crop", "edge:" + edge);
//        Log.i("crop", "dx:" + dx + "|dy:" + dy);
        ...
        if ((edge & GROW_LEFT_EDGE) != 0) {
            if (dx < 0F && r.width() - dx > imageRect.width()) {
                dx = r.width() - imageRect.width();
            }
            if (maintainAspectRatio) {
                dy = dx / initialAspectRatio;
            }
            ...
            if (dy < 0F && r.height() - dy > imageRect.height()) {
                dy = r.height() - imageRect.height();
            }
            if (maintainAspectRatio) {
                dx = dy * initialAspectRatio;
            }
        ...
        final float widthCap = handleRadius * 4;
        float heightCap = maintainAspectRatio
                ? (widthCap / initialAspectRatio)
                : widthCap;
        if ((edge & GROW_LEFT_EDGE) != 0) {
            if (r.left + widthCap > r.right) {
                dx = widthCap - (r.right - r.left);
                if (maintainAspectRatio) {
                    dy = dx / initialAspectRatio;
                }
                r.inset(-dx / 2, -dy / 2);
            ...
            if (r.top + heightCap > r.bottom) {
                dy = heightCap - (r.bottom - r.top);
                if (maintainAspectRatio) {
                    dx = dy * initialAspectRatio;
                }
                r.inset(-dx / 2, -dy / 2);
