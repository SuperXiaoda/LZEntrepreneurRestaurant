...
public class PhotoSelectActivity extends PhotoBaseActivity implements View.OnClickListener, AdapterView.OnItemClickListener {
    ...
    private void setTheme() {
        ...
        mIvEdit.setImageResource(GalleryFinal.getGalleryTheme().getIconEdit());
        if (GalleryFinal.getGalleryTheme().getIconEdit() == R.drawable.ic_gf_edit) {
            mIvEdit.setColorFilter(GalleryFinal.getGalleryTheme().getTitleBarIconColor());
        }
    ...
    protected void takeResult(PhotoInfo photoInfo) {
        ...
        if (!GalleryFinal.getFunctionConfig().isMutiSelect()) { //单选
            ...
            mHanlder.sendMessageDelayed(message, 500);
        } else {//多选
            mSelectPhotoList.add(photoInfo);
            mHanlder.sendMessageDelayed(message, 500);
        }
