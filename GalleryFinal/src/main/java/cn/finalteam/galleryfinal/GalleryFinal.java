...
public class GalleryFinal {
     ...
     * @param callback
     */
    public static void openReview(int requestCode, List<PhotoInfo> photoList, int initPosition, OnHandlerResultCallback callback) {
        FunctionConfig config = copyGlobalFuncationConfig();
        if (config != null) {
            openReview(requestCode, config, photoList, initPosition, callback);
        } else {
            if (callback != null) {
                callback.onHandlerFailure(requestCode, mCoreConfig.getContext().getString(R.string.open_gallery_fail));
            }
     ...
     * @param callback
     */
    public static void openReview(int requestCode, FunctionConfig config, List<PhotoInfo> photoList, int initPosition, OnHandlerResultCallback callback) {
        if (mCoreConfig.getImageLoader() == null) {
            ILogger.e("Please init GalleryFinal.");
            if (callback != null) {
                callback.onHandlerFailure(requestCode, mCoreConfig.getContext().getString(R.string.open_gallery_fail));
        ...
        if (config == null || photoList == null || photoList.isEmpty()) {
            ILogger.d("config为空或文件不存在");
            return;
        }
        mRequestCode = requestCode;
        ...
        mCurrentFunctionConfig = config;
        Intent intent = new Intent(mCoreConfig.getContext(), PhotoReviewActivity.class);
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        intent.putExtra(PhotoReviewActivity.SELECT_MAP, (ArrayList<PhotoInfo>) photoList);
        intent.putExtra(PhotoReviewActivity.INIT_POSITION, initPosition);
        mCoreConfig.getContext().startActivity(intent);
    }
     ...
     * @param photoList
     */
    public static void openReview(List<PhotoInfo> photoList) {
        openReview(photoList, 0);
    }
     ...
     * @param position
     */
    public static void openReview(List<PhotoInfo> photoList, int position) {
        Intent intent = new Intent(mCoreConfig.getContext(), PhotoPreviewActivity.class);
        intent.putExtra(PhotoPreviewActivity.PHOTO_LIST, (ArrayList<PhotoInfo>) photoList);
        intent.putExtra(PhotoPreviewActivity.PHOTO_POSITION, position);
        mCoreConfig.getContext().startActivity(intent);
    }
    ...
    public static void cleanCacheFile() {
        if (mCoreConfig != null && mCoreConfig.getEditPhotoCacheFolder() != null) {
            //清楚裁剪冗余图片
            new Thread() {
                @Override
                public void run() {
                    super.run();
                    try {
                        FileUtils.cleanDirectory(mCoreConfig.getEditPhotoCacheFolder());
